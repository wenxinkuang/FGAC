...
class BasicBlock_SK(nn.Module):
    def __init__(self, in_planes, out_planes, stride, dropRate=0.0):
        ...
        self.convShortcut = (not self.equalInOut) and nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride,
                                                                padding=0, bias=False) or None
        self.fc = nn.Conv2d(in_planes, 10, kernel_size=8, stride =1, bias=False)
...
class NetworkBlock_SK(nn.Module):
    def __init__(self, nb_layers, in_planes, out_planes, block, stride, dropRate=0.0):
        super(NetworkBlock_SK, self).__init__()
        self.nb_layers = nb_layers
        self.layer = self._make_layer(BasicBlock_SK, in_planes, out_planes, nb_layers, stride, dropRate)
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, dropRate):
        ...
        for i in range(int(nb_layers)-1, int(nb_layers)):
            layers.append(BasicBlock_SK(i == 0 and in_planes or out_planes, out_planes, i == 0 and stride or 1, dropRate))
...
class WideResNet(nn.Module):
    def __init__(self, depth=34, num_classes=10, widen_factor=10, dropRate=0.0):
        ...
        # 3rd block
        self.block3 = NetworkBlock_SK(n, nChannels[2], nChannels[3], block, 2, dropRate)
        ...
        self.extra_fc = nn.Conv2d(nChannels[3], 10, kernel_size=8, stride=1, bias=False)
