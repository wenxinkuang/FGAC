...
class SASBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.fc = nn.Conv2d(planes, 10, kernel_size=4, stride=1, bias=False)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        # -------------------------------- debug --------------------------------- #
        self.layer4 = self._make_layer_sk([SASBlock, SASBlock], 512, num_blocks[3], stride=2)
        # -------------------------------- debug --------------------------------- #
        self.linear = nn.Linear(512 * block.expansion, num_classes)

    def _make_layer_sk(self, blocks, planes, num_blocks, stride):
